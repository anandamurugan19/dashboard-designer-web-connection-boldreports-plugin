...
try{
Invoke-Expression "& `"$CAKE_EXE`" `"$Script`" -target=`"$Target`" -configuration=`"$Configuration`" -PreReleaseNumber=`"$revisionNumber`" -studio_version=`"$StudioVersion`" -nugetserverurl=`"$nugetserverurl`" -verbosity=`"$Verbosity`" -nugetapikey=`"$nugetapikey`" -outputpath=`"$OutputPath`" -referencepath=`"$ReferencePath`" $ScriptArgs"
}
catch{
